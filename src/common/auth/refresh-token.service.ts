...
export class RefreshTokenService {
  ...
  async issue(userId: string): Promise<string> {
    ...
    return this.jwtService.signAsync(
      { sub: userId, type: 'refresh', nonce } satisfies RefreshTokenPayload & { nonce: string },
      { expiresIn: this.refreshTtl },
    );
  }

  async rotate(previousToken: string): Promise<string> {
    const payload = await this.verify(previousToken);
    this.revoked.add(this.hash(previousToken));
    return this.issue(payload.sub);
  }

  revoke(token: string): void {
    this.revoked.add(this.hash(token));
  }

  async verify(token: string): Promise<RefreshTokenPayload> {
    if (this.revoked.has(this.hash(token))) {
